...
import { BoardPost, PostCategory, RecyclingAnalysisResult } from '../types';
import {
  addPostToFirestore,
...
const CATEGORIES: PostCategory[] = ['질문하기', '나만의 분리수거 팁', '업사이클링 공유', '자유수다'];
...
export const CommunityBoard: React.FC<CommunityBoardProps> = ({
  ...
  const [category, setCategory] = useState<PostCategory>('나만의 분리수거 팁');
  ...
  useEffect(() => {
    if (sharedAnalysis) {
      setTitle(`[분리수거 가이드] ${sharedAnalysis.itemName} 올바른 배출 방법`);
...
- 난이도: ${sharedAnalysis.difficulty}`);
      setContent(`AI 분석 가이드를 바탕으로 글을 작성합니다.\n\n${sharedAnalysis.summary}`);
    }
  }, [sharedAnalysis]);

  // Subscribe to Firestore posts (or local fallback) ordered by createdAt desc (최신순)
  useEffect(() => {
    setIsLoading(true);
  ...
  const handleSubmitPost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitSuccessMsg('✨ 글이 성공적으로 등록되었습니다!');
      setTimeout(() => setSubmitSuccessMsg(null), 3000);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 rounded-2xl bg-slate-900 text-white shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold">Firestore 게시판 데이터베이스</h3>
              {connected ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  <CheckCircle2 className="h-3 w-3" /> 연결 완료
                </span>
              ) : (
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {connected
                ? 'Firestore에 실시간으로 게시글이 등록 및 조회됩니다.'
                : '붙여넣은 firebaseConfig 설정값으로 연결할 수 있습니다. (서비스 계정 키 불필요)'}
            </p>
          </div>
        </div>
        ...
        >
          {connected ? '⚙️ Firebase Config 변경' : '🔥 Firebase WebConfig 붙여넣기'}
        </button>
      </div>
          ...
          <div className="mb-4 p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 dark:bg-emerald-950/40 dark:border-emerald-900 text-xs">
            <div className="flex items-center justify-between mb-1">
              <span className="font-bold text-emerald-900 dark:text-emerald-300 flex items-center gap-1">
                <Sparkles className="h-3.5 w-3.5" /> 첨부된 AI 분리수거 분석 결과
              </span>
              <button
                type="button"
        ...
        <form onSubmit={handleSubmitPost} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="분리수거 관련 궁금증, 나만의 꿀팁, 업사이클링 경험을 나눠주세요!"
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              <span>{isSubmitting ? 'Firestore 저장 중...' : '게시글 등록하기'}</span>
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 px-1">
          <span>총 <strong>{filteredPosts.length}</strong>개의 게시글 (최신순)</span>
        </div>
        ...
        {isLoading ? (
          <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-500">Firestore에서 최신 게시글을 불러오는 중...</p>
          </div>
        ) : filteredPosts.length === 0 ? (
          <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800">
            <MessageSquareText className="h-10 w-10 text-slate-300 dark:text-slate-600 mx-auto mb-2" />
            <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
              등록된 게시글이 없습니다.
            </p>
            <p className="text-[11px] text-slate-400 mt-1">첫 번째 분리수거 팁이나 질문글을 등록해 보세요!</p>
          </div>
        ) : (
